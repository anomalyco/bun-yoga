...
if (missingRequired.length > 0) {
  console.error(
    `Error: Missing required fields in package.json: ${missingRequired.join(
      ", "
    )}`
  );
  process.exit(1);
...
if (buildNative) {
  ...
  for (const { platform, arch } of variants) {
    ...
    for (const name of ["libyoga", "yoga"]) {
    ...
    if (copiedFiles === 0) {
      ...
      console.error(
        `Expected to find files like: libyoga.so, libyoga.dylib, yoga.dll`
      );
...
if (buildLib) {
  ...
  for (const filePath of bundledFiles) {
    ...
    if (existsSync(fullPath)) {
      ...
      const helperExportPattern =
        /^export\s*\{([^}]*(?:__toESM|__commonJS|__export|__require)[^}]*)\};\s*$/gm;
      ...
      content = content.replace(helperExportPattern, (match, exports) => {
        ...
        const nonHelpers = exportsList.filter(
          (e: string) => !helpers.includes(e)
        );

        if (nonHelpers.length > 0) {
          modified = true;
          const helperExports = exportsList.filter((e: string) =>
            helpers.includes(e)
          );
