...
console.log(`Publishing @opentui/yoga@${packageJson.version}...`);
...
for (const pkgName of Object.keys(
  packageJsons[libDir].optionalDependencies!
...
Object.entries(packageJsons).forEach(([dir, { name, version }]) => {
  ...
  const publish: SpawnSyncReturns<Buffer> = spawnSync(
    "npm",
...
console.log(`\nAll @opentui/yoga packages published successfully!`);
