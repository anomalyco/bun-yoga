...
function getCacheDir(owner: string, repo: string, releaseTag: string): string {
  return path.join(tmpdir(), 'gh-releases-cache', `${owner}-${repo}`, releaseTag);
}

function getAssetCacheDir(owner: string, repo: string, releaseTag: string, assetName: string): string {
  return path.join(getCacheDir(owner, repo, releaseTag), assetName);
}

async function isAssetCached(owner: string, repo: string, releaseTag: string, assetName: string): Promise<boolean> {
...
async function getCachedAssetPath(owner: string, repo: string, releaseTag: string, assetName: string): Promise<string | null> {
  const cacheDir = getAssetCacheDir(owner, repo, releaseTag, assetName);
  try {
    const files = await fs.readdir(cacheDir);
    if (files.length > 0) {
...
function parseAssetName(assetName: string): { platform: string; arch: string; linkage: string } | null {
  const match = assetName.match(/yoga-.*?-(darwin|linux|windows)-(x86_64|arm64|aarch64|x64|ARM64)-(static|shared)\.tar\.gz$/);
...
async function main(args: Args) {
  ...
  console.log('Fetching release assets for:', `${args.owner}/${args.repo}@${args.releaseTag}`, 'Linkage:', args.linkage);
  ...
  try {
    ...
    if (listProc.exitCode !== 0) {
      console.error(`Error getting release info with gh api. Exit code: ${listProc.exitCode}`);
    ...
    const data = JSON.parse(listOutput) as { assets: any[], message?: string, tag_name?: string };

    if (data.message && data.message.toLowerCase().includes("not found")) {
        console.error(`Error: Could not find release "${args.releaseTag}". Message: ${data.message}`);
        process.exit(1);
    }
    ...
    console.log(`Found ${data.assets.length} assets in release ${data.tag_name}`);
    ...
    for (const asset of data.assets) {
      ...
      const parsed = parseAssetName(assetName);
      if (!parsed) {
        console.log(`Skipping asset with unrecognized name pattern: ${assetName}`);
        continue;
      }
    ...
    if (assetsToProcess.size === 0) {
      if (args.platform) {
        console.log(`No assets found matching platform '${args.platform}' and linkage '${args.linkage}'.`);
      } else {
    ...
    for (const item of assetsToProcess) {
      ...
      console.log(`Processing asset: ${assetName}, ID: ${assetId}, Size: ${assetSizeBytes} bytes`);
      console.log(`Target directory: ${assetDir}`);

      let downloadedArchivePath: string;
      
      try {
        const isCached = await isAssetCached(args.owner, args.repo, args.releaseTag, assetName);
        
        if (isCached) {
          const cachedPath = await getCachedAssetPath(args.owner, args.repo, args.releaseTag, assetName);
          if (cachedPath) {
          ...
          const assetCacheDir = getAssetCacheDir(args.owner, args.repo, args.releaseTag, assetName);
          await fs.mkdir(assetCacheDir, { recursive: true });
          console.log(`Created cache directory: ${assetCacheDir}`);

          console.log(`Downloading "${assetName}" using gh release download...`);
          ...
          if (ghDownloadProc.exitCode !== 0) {
            console.error(`Error downloading asset "${assetName}" with gh release download. Exit code: ${ghDownloadProc.exitCode}`);
          ...
          console.log(`Asset "${assetName}" downloaded by gh to ${assetCacheDir}`);
          ...
          if (filesInCacheDir.length === 0) {
            console.error(`No files found in cache directory: ${assetCacheDir}`);
            continue;
          }
          if (filesInCacheDir.length > 1) {
            console.warn(`Multiple files found in cache directory: ${assetCacheDir}. Using the first one: ${filesInCacheDir[0]}`);
          }
          const downloadedArchiveFileName = filesInCacheDir[0]!;
          downloadedArchivePath = path.join(assetCacheDir, downloadedArchiveFileName);
        ...
        console.log(`Unpacking ${downloadedArchivePath} to ${assetDir} using tar...`);
        ...
        if (tarProc.exitCode !== 0) {
          console.error(`Error unpacking "${assetName}" with tar. Exit code: ${tarProc.exitCode}`);
    ...
    console.log(`\nAssets cached in: ${getCacheDir(args.owner, args.repo, args.releaseTag)}`);
  } catch (error) {
...
if (import.meta.main) {
  ...
  const defaultReleaseTag = 'v3.2.1-test.2';
  ...
  program
    ...
    .option('-t, --releaseTag <tag>', 'Release tag (e.g., v3.2.1-test.2)', defaultReleaseTag)
    .option('-l, --linkage <type>', "Linkage type: 'static' or 'shared'", defaultLinkage)
    .option('-p, --platform <name>', 'Optional: Specific platform to download (e.g., darwin, linux, windows)')
    .action((options) => {
      ...
      if (linkage !== 'static' && linkage !== 'shared') {
        console.error(`Invalid linkage type "${linkage}". Must be "static" or "shared".`);
        program.help();
      }

      main({ owner, repo, releaseTag, linkage: linkage as 'static' | 'shared', platform });
    });
